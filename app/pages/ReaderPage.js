...
export default class ReadPage extends Component {
  ...
  getArticle(type, params) {
    ...
    http('http://172.30.66.50:8080/api/book/content?url=http://www.bqg99.com/book/1450365435/1077495642.html',{
        success:(data)=>{
    
            this.setState(pre=>{
                pre.content = JSON.parse(data)
                return{
                    article: pre
                }
            })
        ...
        <ScrollView
          ...
          <TouchableOpacity
            ...
            <View
              ...
              <View style={[styles.content, this.state.readStyle.content]}>
                {this.state.article.content.map((text,index)=>{

                    return(
                      <Text key={index} style={[
                        { fontSize: this.state.fontSizeStyle.contentFontSize, lineHeight: this.state.fontSizeStyle.titleFontSize + 10 }]}>{text}</Text>
                    )
                })}
              </View>
